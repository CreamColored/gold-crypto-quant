...
行情页在K线之上增加了**实时盘口**：两所三品种的买一卖一、基差和本秒帧数。数据来自盘口
采集器写入的秒级聚合表，**不是浏览器直连交易所**——展示的是"最近一秒的极值"，因此每行
显式给出数据年龄，超过10秒标红。采集器停了页面必须看得出来，而不是继续显示几小时前的
价格。基差是双所对照实验的关键观测量：2026-09-03 13:23 那笔 ETH，Gate 差 0.38 点没触到
轨、币安穿了 0.41 点，两个账户就此分叉了 115U。

**页面不会模仿交易所的下单面板。** Gate 交易页的委托区、买卖按钮一概不做——Web路由里
不存在开仓、平仓、撤单或策略修改接口，这是硬约束。

...
- 当前策略：布林带震荡箱体轨道轮转 `MULTI_ROTATION_STRATEGY_VERSION = 5.8.0`
...
python main.py public-market-comparison --poll-seconds 20   # 量化服务影子服务，默认7天
...
| 量化服务（public-market-comparison） | **零网络IO**，每秒读 Redis 判断有没有新收线，有才跑策略 |
| Web | 只读监管数据 |

量化服务每秒只读一个 cursor Hash（约0.5毫秒）判断有没有新K线；60轮里约59轮属于
...
1秒轮询下每轮一行是一天86400行、约16MB，其中绝大多数是"什么都没发生"，真出事的
那几行反而找不到。默认只在**有新收线K线**或**产生了交易信号**时留痕，并在这一行
带上两次留痕之间的心跳统计：

```
量化服务第125轮完成：...；近60轮：在途观察12轮触轨外、最长连续2秒
```

停留确认要求连续3秒，"最长连续2秒"就说明它挡住了一次插针——**没有这个统计就完全
看不出那59轮在干什么**，日志里只剩"权益没变、信号0条"。

盯盘或排查时给启动脚本加 `--log-every-cycle` 逐轮输出，每行标出这一轮的性质：

```
第25轮◆新K线    ← 60轮里只有这1轮
第26轮·在途复查  ← 其余59轮，停留确认靠它们计时
```

### 宏观数据发布前后暂停开仓

2026-09-04 20:30:00 美国非农发布（实际 +16.2万，预期 +5.3万，超出三倍），BTC 一分钟
内从 81285 砸到 80130——振幅1155点、成交量放大12倍。影子账户在 20:30:3x 因为"跌破
下轨且连续3秒还在轨道外"判定为有效触碰，开了两笔多单，同一分钟内双双止损，
共亏 49.36U。

**停留确认在这类行情里起反作用。** 它的前提是"持续触碰比瞬时插针可信"，而在数据
驱动的单边启动里，持续待在轨道外恰恰说明压力还在，不是要回归。

**箱体的量比也拦不住。** 量比看的是已收线K线：12:29 那根量只有78万、一切正常，
爆量发生在 12:30 这根之内；而在途K线没有成交量（盘口推送里不含），当根爆量在收线
前根本不可见。

所以不从形态上识别，改从**时间**上规避——发布时刻是已知的，**时间是确定的，形态是
猜的**。发布前后各静默 15 分钟（`MACRO_BLACKOUT_BEFORE_MINUTES` /
`MACRO_BLACKOUT_AFTER_MINUTES`，设 0 关闭）。发布前也要挡：数据出来前盘口就开始
变薄，那时候的触轨同样不是震荡回归。

事件表在 `config/macro-events.json`。非农按"每月第一个周五美东8:30"自动展开；
CPI 和 FOMC 每月日期不同，需要手工维护。**时间一律填美东时间**——8:30 ET 在夏令时是
12:30 UTC、冬令时是 13:30 UTC，写死 UTC 偏移会让窗口半年错整整一个小时而且不报错。

静默挂在与交易开关同一个 `entry_allowed` 上，因此**自动覆盖所有开仓路径（含对侧轨
反手），也自动不影响止损、减仓与止盈**——静默是为了少开单，不是让风险动作停摆。

### 发版与远程排查

`scripts/deploy.sh` 从 Mac 一条命令发版。**不做 push 自动部署**：这套跑的是交易逻辑，
push 之后立刻生效等于少了一道人为确认。脚本里每一步都有门——测试不过不发、有未提交
改动不发、有持仓时要确认、代码没变化就跳过重启、依赖只在 pyproject 变了才装。

验证不能只看 `systemctl is-active`：崩溃重启循环里它也会短暂显示 active。必须确认
服务真的产出了新日志，否则"进程在但卡住了"看起来和正常一模一样。

`scripts/remote.sh` 是排查入口。**不把日志传回 Mac**——那是在搭基础设施解决一个不
存在的问题，ssh 过去读就是实时的。

真正有用的是 `remote.sh tunnel`：把服务器的 MySQL 与 Redis 转发到本机
（13306 / 16379，刻意避开本机同名服务的端口），本机所有诊断脚本不用改一行就能对着
服务器的数据跑。今天排查那两次故障靠的都是现场查库查缓存，不是翻日志。

### 启用哪些交易所与品种

`.env` 的 `ACTIVE_VENUES` 与 `ACTIVE_SYMBOLS` 决定实际跑哪些，留空即全部启用。
**停掉的那路连 WebSocket 盘口订阅与 REST 拉取一起停**，不只是不开仓——省掉对应的
流量与策略计算。

做成配置而不是改代码，是为了开回来时不用改代码：品种表 `ALL_COMPARISON_CONTRACTS`
始终保留三个品种，有测试守着不被误删。

停跑的交易所在对照摘要里显示"已停用"而不是"本轮行情异常"。**停跑是配置，不是
故障**——每分钟报一次假异常会让真正的故障淹没在噪声里。

### 服务启停不推送

重启是日常操作——改造期间一天重启了六七次。每次都推邮件和钉钉，真正的异常会淹在
噪声里。启停只写日志；**行情中断、单轮超时、Redis降级这些异常告警照常推送**，
恢复也各推一次。
...
K线加载同理按水位缓存：收线水位没变说明这段K线一根都没动，直接复用，省掉每秒重读
5700根的35毫秒。没有水位时一律不缓存——无法判断数据是否变过时，宁可每次重读也不能
拿旧数据冒充新的。

线上每秒一轮的CPU占用从 46.5% 降到 15.2%；**逐秒回放24小时从51分钟降到5分钟**，
后者才让"在途K线的回测"从不可行变成可行。

### 从20秒改到1秒时被放大的三处

把轮询从20秒改成1秒，所有"每轮做一次"的动作都放大了20倍。只优化指标计算是不够的，
另外三处是逐个量出来的：

| | 问题 | 改法 |
|---|---|---|
| 行情健康检查 | 单轮76条SQL里75条是它，且运行器与策略周期各做一遍 | 去掉重复，跟着收线走 |
| 影子快照 | 表按分钟去重，同一行每分钟被 upsert 60 次 | 只在有新收线或有事件时写 |
| `read_bars` | 取全量再在Python里切，读2根要解析501条 | 用负索引直接取尾部 |

归因办法是逐个停服务量：停掉量化服务后 MySQL 查询速率从 348/秒 掉到 4.3/秒，
说明九成负载在它身上；再用 SQLAlchemy 的 `before_cursor_execute` 数出单轮76条。

结果：MySQL 查询 348/秒 → 12.6/秒，容器CPU 14.5% → 2.1%，Redis 出站
348 KB/秒 → 66 KB/秒。量化三个服务合计占全机算力约 1.6%。
...
**量化服务按前台运行**，Terminal 里能看到实时日志，关掉窗口即停止；Web 后台常驻即可。
...
```
[2026-09-03 15:35:56 +0800] 量化服务第10轮完成：Gate刷新7485根、权益9876.03U、信号0条；
...
UTC。`.env` 不纳入版本库（见 `.gitignore`），新环境执行 `cp .env.example .env` 后填写；
`.env` 含明文密钥，不得提交、公开分享或写进日志。新增配置项时同步更新 `.env.example`。
...
**5分钟不再直接触发交易。** 开仓优先级变成 15m → 30m → 1h。5m 仍留在
`INTERVAL_PRIORITY` 里——顶底结构要查全部七个周期，把它从那里拿掉会连结构判定一起
丢掉；只是它不再出现在 `ENTRY_INTERVAL_PRIORITY` 中。已持有的5m仓位照常管理到结束。

**减仓比例分两档。** 第一次减仓落在中轨，无论有没有顶底结构都是 30%；第二次及以后
（对侧轨结构确认、结构延续阶梯）仍是 50%。**止损位置与提前止损逻辑一律不动。**

**止损改为不分周期的固定点数：** 大饼 300、二饼 12，其余品种按125倍杠杆下浮亏100%
换算（价格反向波动 0.8%）。

仓位是按 `equity × risk_per_trade ÷ 止损距离` 反推的，所以**放宽止损等于缩小仓位，
每单风险仍是账户的 0.25%**——止损变宽不会让单笔亏得更多，只会让持仓更抗震荡。

| 品种 | 周期 | 旧止损 | 新止损 | 仓位变化 |
|---|---|---|---|---|
| BTC | 15m | 250 | 300 | −17% |
| BTC | 30m/1h | 500 | 300 | +67% |
| ETH | 15m | 5 | 12 | −58% |
| ETH | 30m/1h | 10 | 12 | −17% |
| XAU | 全部 | 14.3 | 35.8 | −60% |
...
当前唯一活动策略是 `MULTI_ROTATION_STRATEGY_VERSION = 5.8.0`。每个交易所独立对BTC和
...
两个交易所都支持。用项目自己的交易所客户端发请求，与量化服务走完全相同的代码路径和代理
...
Tailscale等安全内网并显式设置 `WEB_HOST`，不要把该端口直接暴露到公网。Web服务和量化服务
模拟服务是两个职责独立的进程：前者只读展示，后者继续采集行情和维护影子账户；关闭Web页面
不会停止模拟交易。

量化服务每分钟把两套账户的权益、峰值、当日起始权益、持仓数量和风控状态写入

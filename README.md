...
行情页右侧显示**当前选中交易所与品种的实时盘口**，分两路显示，缺一不可：

| | 来源 | 作用 |
|---|---|---|
| 大号价格 | **浏览器直连交易所 WebSocket** | 亚秒级跳动，看市场此刻的真实价格 |
| 下方小字 | 采集器写入的秒级聚合表 | 带数据年龄，采集器停了必须看得出来 |

**只留直连是不行的**：采集器挂掉时直连价格照样跳，页面一切正常，而策略用的正是采集器
那条链路，故障会被完全掩盖——页面就从"系统监控"退化成"价格显示器"。今天 cloudflared
进程活着、隧道却是死的，就是这类静默故障。

直连实测：Gate 47 帧/秒、币安 1106 帧/秒。**渲染必须与收帧解耦**，逐帧改 DOM 会让页面
持续重排；现在按固定1秒节奏渲染最后一帧。切换交易所或品种会重连，断线按指数退避重连，
离开页面时关闭连接。

直连也意味着**依赖访问者所在网络能连上交易所**——手机用流量在外网可能连不上，此时大号
价格会显示"连接断开"，而下方采集器那一路仍然正常，页面不会整块失效。

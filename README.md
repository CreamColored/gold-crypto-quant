...
开仓时会检查该仓位方向对应的MACD背离，命中后这笔仓位到对侧轨不再止盈反手，改走阶梯延续。

- **顶部结构**＝MACD顶背离：后一个摆动高点收盘价更高，但对应DIF更低（做空方向确认）
- **底部结构**＝MACD底背离：后一个摆动低点收盘价更低，但对应DIF更高（做多方向确认）
- 判定用标准MACD(12,26)的DIF快线；摆动点需左右各2根K线确认，因此最近2根不参与判定
- 检测范围：该仓位所在主周期，或1分钟、3分钟序列，任一命中即成立

命中后的仓位管理（以空单为例，多单对称）：

1. 中轨：减仓50%，止损移到开仓价
2. 下轨（对侧轨）：**不止盈也不反手**，再减仓50%，止损收紧到中轨
3. 此后每继续有利移动一个步长：减仓50%，止损跟到**上一个**减仓价，与当前价保持一个步长缓冲
4. 一直延续到止损被打出为止；止损只会越收越紧，不会差于中轨那一档

阶梯步长在ETH价位是8点，BTC和XAU按最近20根收盘中位数相对2,500等比例放大，使三个品种
都落在价格的0.32%附近——8点直接套用到BTC只有0.01%，属于噪音级别。
...
每一笔本地模拟买入成交、卖出成交、部分止盈和平仓都会分别发送邮件；止损成交使用严重
告警级别。服务启动、停止、首次连接异常、异常恢复、账户触发或解除熔断、行情健康阻断、
策略准入阻断和执行层阻断也会即时发送。相同持续状态在单次服务运行中只通知一次，连续
重试只在首次失败和恢复时通知，避免一分钟轮询反复刷屏。每封事件邮件附带当时的账户风控、
持仓、模拟权益以及Mac的CPU、内存、磁盘和进程占用；邮件审计不保存正文或SMTP授权码。
...
开仓前检查反方向结构，命中即跳过这一单：做多遇顶背离不开、做空遇底背离不开。首次触轨
开仓和对侧轨止盈后的反手都适用。检测范围与阶梯延续一致——该仓位周期或1分钟、3分钟序列。

用9月2日当天19笔影子开仓回放，该规则否决其中15笔、合计 -200.80 USDT，当天全部5笔止损
单均被拦下。样本仅一个单边下跌交易日、否决率高达79%，横盘震荡日可能连正常箱体波段一起
过滤掉，因此这个数字只说明方向，不代表预期收益。

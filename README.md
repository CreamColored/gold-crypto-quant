...
- 策略：15分钟布林带箱体轨道轮转 `BOLLINGER_RANGE 3.0.0`
- 方向：多空双向
- 加密货币：官方合约测试网
...
策略只使用已经收盘的15分钟K线计算下一根可用轨道，避免未来数据。
125 倍仅为保证金杠杆上限，仓位由账户风险和止损距离决定。

## 当前布林带策略

当前唯一活动策略是 `BOLLINGER_RANGE 3.0.0`。它只运行ETH 15分钟箱体轨道轮转：布林带
使用20周期、2倍标准差；最近连续3根已收盘K线中，上轨、中轨和下轨各自漂移不超过2.5
USDT时建立箱体候选。下一根K线触及上轨做空、触及下轨做多；中轨减仓50%并保护到开仓价，
对侧轨全部止盈并立即反手。固定止损为5 USDT；止损后暂停，重新连续出现3根箱体候选K线
才恢复。连续两根收在轨道外确认箱体突破。所有成交只存在本地影子账户，Gate订单接口始终
不会被调用。

详细研究结果见 [ETH15分钟轨道轮转30天回测](docs/ETH15分钟轨道轮转30天回测.md)。`backtest-bollinger` 使用
...
`BOLLINGER_RANGE 3.0.0`，旧EMA和旧版布林带记录都不能再授权Gate模拟开仓。
